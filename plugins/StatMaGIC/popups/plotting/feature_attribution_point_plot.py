...
class featAttPlot(QDialog):
    def __init__(self, parent):
        ...
        # self.raster_select_layout.addRow(raster_label, self.raster_selection_box)
        ...
        explanation_text = QLabel('Interpretation: Positive values are associated with a positive prediction '
                                  'and negative values associated with a negative prediction.'
                                  'the magnitude of the value is the strength of the association')
    ...
    def extract_values(self):
        ...
        x_pos, y, ticks = merge_band_list_and_value_dict(band_list, value_dict, do_sort=do_sort)
    ...
    def update_plot(self, x_pos, y, ticks):
        # https://stackoverflow.com/questions/72002352/pyqtgraph-how-to-name-bars-in-bargraphitem
        # self.bargraph = pg.BarGraphItem(x=x_pos, height=y, width=0.5)
...
def merge_band_list_and_value_dict(band_list, value_dict, do_sort=True):
    x_labs = []
    for band in band_list:
        band_split = band.split(':')
        if len(band_split) == 1:
            x_labs.append(band_split[0])
